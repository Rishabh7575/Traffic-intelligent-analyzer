...
def format_progress_bar(current, total, width=30):
    if not total:
        return f"{current / (1024 * 1024):6.1f} MB"
...
def download_with_progress(url, destination_path):
    ...
    with urlopen(request, timeout=60) as response, open(destination_path, "wb") as file_handle:
        ...
        while True:
            ...
            progress = format_progress_bar(downloaded, total_size)
            if total_size:
                sys.stdout.write(f"\rDownloading yolov3.weights {progress}")
            else:
                sys.stdout.write(f"\rDownloading yolov3.weights {progress}")
            sys.stdout.flush()
    sys.stdout.write("\n")
...
def ensure_weights_file(weights_path):
    ...
    for url in WEIGHTS_URLS:
        try:
            print(f"Weights file not found or invalid ({reason}). Downloading yolov3.weights...")
            print(f"Source: {url}")
            if temp_path.exists():
                temp_path.unlink()
    ...
    raise SystemExit(
        ...
        f"Last error: {last_error or reason}\n"
...
def arg_parse():
    ...
    parser.add_argument("--images",
                        ...
                        default=str(resolve_path("vehicles-on-lanes")),
                        type=str)
    ...
    parser.add_argument("--cfg",
                        dest='cfgfile',
                        help="Config file",
                        default=str(resolve_path("config/yolov3.cfg")),
                        type=str)
    parser.add_argument("--weights",
                        dest='weightsfile',
                        help="weightsfile",
                        default=str(resolve_path("weights/yolov3.weights")),
                        type=str)
    parser.add_argument(
                        "--reso",
                        dest='reso',
...
classes = load_classes(str(resolve_path("data/idd.names")))

#***Setting up the neural network***
model = Darknet(str(resolve_path(args.cfgfile)))
print('\033[0m' + "Input Data Passed Into YOLO Model..." + u'\N{check mark}')
weights_path = ensure_weights_file(resolve_path(args.weightsfile))
...
try:
    ...
    if images_path.is_dir():
        imlist = [str(item) for item in sorted(images_path.iterdir()) if item.is_file()]
    elif images_path.is_file():
        imlist = [str(images_path)]
    else:
